...
MaybeLocal<Value> URLPattern::URLPatternInit::ToJsObject(
  ...
  if (init.protocol) {
    ...
    USE(result->Set(context, env->protocol_string(), protocol));
  }
  if (init.username) {
    ...
    USE(result->Set(context, env->username_string(), username));
  }
  if (init.password) {
    ...
    USE(result->Set(context, env->password_string(), password));
  }
  if (init.hostname) {
    ...
    USE(result->Set(context, env->hostname_string(), hostname));
  }
  if (init.port) {
    Local<Value> port;
    ...
    USE(result->Set(context, env->port_string(), port));
  }
  if (init.pathname) {
    ...
    USE(result->Set(context, env->pathname_string(), pathname));
  }
  if (init.search) {
    Local<Value> search;
    ...
    USE(result->Set(context, env->search_string(), search));
  }
  if (init.hash) {
    Local<Value> hash;
    ...
    USE(result->Set(context, env->hash_string(), hash));
  }
  if (init.base_url) {
    ...
    USE(result->Set(context, env->base_url_string(), base_url));
  }
  return result;
}
