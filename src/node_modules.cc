...
void BindingData::GetNearestParentPackageJSON(
  ...
  do {
    ...
    // to walk upwards
    if (enabled_permission && env->permission()->is_granted(
                                  permission::PermissionScope::kFileSystemRead,
    ...
    if (check_path.size() >= kNodeModules.size() &&
        std::equal(
            check_path.begin() + check_path.size() - 1 - kNodeModules.size(),
            check_path.end(),
            kNodeModules.begin())) {
      return;
    }
...
void BindingData::GetNearestParentPackageJSONType(
  ...
  auto node_modules_suffix = kPathSeparator + std::string("node_modules");
  std::string_view check_path = path.ToStringView();
  size_t root_separator_index = check_path.find_first_of(kPathSeparator);
  ...
  do {
    ...
    auto check_path_with_sep =
        std::string(check_path.substr(0, separator_index)) + kPathSeparator;

    // Stop the search when the process doesn't have permissions
    // to walk upwards
    if (UNLIKELY(!realm->env()->permission()->is_granted(
            permission::PermissionScope::kFileSystemRead,
            check_path_with_sep))) {
      return;
    }

    if (check_path.size() > node_modules_suffix.size() &&
        strcmp(
            check_path.data() + check_path.size() - node_modules_suffix.size(),
            node_modules_suffix.data()) == 0) {
      return;
    }
