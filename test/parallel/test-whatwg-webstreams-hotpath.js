...

{
  ...
  const rs = new ReadableStream();
  rs.cancel().then(common.mustCall(), common.mustNotCall());
}

{
  ...
  const rs = new Sub();
  assert.ok(rs.locked);
  rs.reader.cancel().then(common.mustCall(), common.mustNotCall());
}

{
  ...
  const rs = new Sub();
  rs.closed.then(common.mustCall(), common.mustNotCall());
}
...

{
  const rs = new ReadableStream();
  const reader = rs.getReader();
  reader.cancel().then(common.mustCall(), common.mustNotCall());
}
...

{
  ...
  pa.then(common.mustCall(), common.mustNotCall());
...

{
  ...
  const rs = new ReadableStream();
  rs[kControllerErrorFunction](new Error('empty-error'));
  assert.strictEqual(rs[kState].controller, undefined);
  rs.cancel().then(common.mustNotCall(), common.mustCall());
  assert.strictEqual(rs[kState].controller, undefined);
}
