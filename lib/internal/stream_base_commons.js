...
// Uint8Array[] already in stream/iter batch form. Unlike writevGeneric,
// this does not expect { chunk, encoding } descriptors and does not mutate
// the input array.
function writevBuffers(self, buffers, cb) {
  const handle = self[kHandle];
  const ret = handle.writev(null, buffers, true);
  return afterWriteDispatched(handle, ret, buffers, cb);
}

function writeOneBuffer(self, buffer, cb) {
  const handle = self[kHandle];
  const ret = handle.writeBuffer(null, buffer);
