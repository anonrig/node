...
function deserializePackageJSON(path, contents, checkIntegrity = false) {
  ...
  if (checkIntegrity) {
    ...
    checkPackageJSONIntegrity(path, manifest);
  }
...
function getNearestParentPackageJSON(checkPath) {
  const result = modulesBinding.getNearestParentPackageJSON(toNamespacedPath(checkPath));

  if (result === undefined) {
    return false;
  }

  return deserializePackageJSON(checkPath, result, true /* checkIntegrity */);
}
