    ...
    for (const batch of pipeline) {
      ...
      if (hasWritevSync && batch.length > 1) {
        ...
        for (let i = 0; i < entry.views.length; i++) {
          ...
          const chunk = i === 0 ? view.value : validateByteView(view);
...
async function pipeTo(source, ...args) {
  ...
  function writeBatch(batch) {
    ...
    if (hasWritev && batch.length > 1) {
      const chunks = validateBatchEntry(entry);
      if (!hasWritevSync || !writer.writevSync(chunks)) {
        const opts = signal ? { __proto__: null, signal } : undefined;
        const writevResult = writer.writev(chunks, opts);
        if (writevResult === undefined) {
          assertBatchEntry(entry);
          totalBytes += entry.byteLength;
          return;
        }
        const thenable = writevResult !== null &&
          typeof writevResult.then === 'function' ?
          writevResult :
          PromiseResolve(writevResult);
        return PromisePrototypeThen(thenable, () => {
          assertBatchEntry(entry);
          totalBytes += entry.byteLength;
        });
      }
    ...
    for (let i = 0; i < entry.views.length; i++) {
      const view = entry.views[i];
      // The batch was just snapshotted; the first view cannot have been
      // resized by a previous writeSync in this loop.
      const chunk = i === 0 ? view.value : validateByteView(view);
